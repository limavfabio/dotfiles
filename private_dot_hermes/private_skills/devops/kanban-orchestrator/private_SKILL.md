...
description: Decomposition playbook + anti-temptation rules for an orchestrator profile routing work through Kanban. Includes the implementation→review two-card pattern with decision tree. The "don't do the work yourself" rule and the basic lifecycle are auto-injected into every kanban worker's system prompt; this skill is the deeper playbook when you're specifically playing the orchestrator role.
version: 3.0.1
...
**Implementation → Review (two-card):** Every non-trivial implementation gets a paired review card. Both assigned to the SAME profile — context isolation comes from the fresh kanban run, not from profile differences. (If you want a different model/config for review, create a separate profile; most setups don't need this.)

Implementation card shape:

```python
impl = kanban_create(
    title="implement: <feature name>",
    assignee="<profile>",  # whichever profile handles implementation on this setup
    body="""<task spec, acceptance criteria, file paths, constraints>

WORKFLOW:
1. Read the parent planner card's metadata for full context
2. Create worktree + feature branch (see kanban-worker skill)
3. Implement following TDD: write failing test → minimal code → passing test
4. Run full test suite — zero regressions
5. Self-review diff: no stray files, no config pollution, no scope creep
6. Commit with descriptive message
7. Complete with summary + metadata (changed_files, tests_run, branch, decisions)

WHEN DONE:
- Use kanban_complete(). The review card auto-promotes to ready.
""",
...
Review card shape (parent = implementation card):

```python
review = kanban_create(
    title="review: <feature name>",
    assignee="<profile>",  # same profile — fresh run = fresh eyes
    body="""Read the parent implementation card's completion metadata.
Clone the branch and review EVERY changed file.

CHECK:
- Spec compliance: does it match the task spec?
- Code quality: conventions, error handling, naming, edge cases
- Security: no hardcoded secrets, no injection, safe deserialization
- Tests: coverage adequate? all passing? no regressions?
- Cleanliness: no stray files modified, no config pollution, no scope creep
- Branch hygiene: unique branch name, pushed, not on shared ref

OUTPUT (via kanban_complete metadata):
{
  "verdict": "PASS" | "CHANGES_REQUESTED",
  "findings": [{"severity": "critical|important|minor", "file": "...", "line": N, "issue": "..."}],
  "branch": "task/<slug>",
  "pr_number": N  // if PR was opened
}

If PASS → complete the review card. Chain ends.
If CHANGES_REQUESTED → complete the review card (NOT block), then create a
remediation card (see decision tree below).
""",
...
```
Review card completes
├── verdict: PASS
│   └── Done. Nothing more to create.
│
└── verdict: CHANGES_REQUESTED
    ├── findings are critical (security, data loss, spec violation)
    │   └── Create remediation card, assigned to same profile,
    │       parents=[review_id], skills=["subagent-driven-development"]
    │       body="Fix these specific findings from the review card: ..."
    │       After remediation completes, create a NEW review card
    │       (parents=[remediation_id]) for re-review.
    │       └── Re-review passes → done.
    │       └── Re-review fails → another remediation (max 2 cycles, then escalate to user)
    │
    └── findings are minor (naming, style, non-blocking)
        └── Create a follow-up card for later (no parent link, same profile,
            skills=["subagent-driven-development"])
            and mark the feature as done. Don't block the chain on minors.
```

**Why two cards instead of block/unblock:** `kanban_block` is for human-in-the-loop. Review is an agent-in-the-loop gate — it deserves its own card with its own lifecycle, metadata, and traceability. Two cards also means the implementation worker doesn't sit idle waiting for review; it completes and frees the profile for other work.

**Why same profile works:** A kanban spawn is a fresh process with a clean context window. The review worker has zero shared state with the implementation worker — it only sees the handoff metadata. This is the same principle as `subagent-driven-development`'s independent reviewer subagent, but with board persistence.

**When to skip the review card:** Documentation-only changes, pure config tweaks, tasks under ~5 lines of code. Use judgment — the cost of a review card is one extra kanban spawn. The cost of skipping it is undetected regressions.
...
- **`references/cli-fallback.md`** — CLI equivalents for all kanban tool calls. Load when the agent does NOT have kanban tools in its schema (e.g., main session agent using `hermes kanban` CLI instead of `kanban_create()` tool calls).
- **`references/dispatcher-rules.md`** — Verified status transition rules: `todo → ready` (auto when all parents done), `triage` (dispatcher ignores until human/specifier promotes).
- **`references/operational-notes.md`** — Field notes: changing workspace post-creation (SQLite), `specify` command internals, `--no-promote` patch.
